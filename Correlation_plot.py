...
index_state[51] = 'PR'
states = index_state.values()
# hashmap = defaultdict(list)
date_ls = list()
correlation_ls = list()
correlation_ls_in = list()
for i in range(0, 142):
    start_date = datetime.strptime('2022-04-30', "%Y-%m-%d") + timedelta(days=i*7)
    date_ls.append(start_date.strftime("%Y-%m-%d"))
    temp_file = pd.read_csv('metrics_of_MTE/metrics_by_{}.csv'.format(start_date.strftime("%Y-%m-%d")))
    p_value_out = temp_file['causal_out_strength_p_value'].values[0]
    cor_value_out = temp_file['causal_out_strength_correlation'].values[0]

    p_value_in = temp_file['causal_in_strength_p_value'].values[0]
    cor_value_in = temp_file['causal_in_strength_correlation'].values[0]

    if math.isnan(cor_value_out):
        cor_value_out = 0
    if p_value_out > 0.05:
        #for state in states:
            #hashmap[state].append(0)
        correlation_ls.append(0)
    else:
        correlation_ls.append(cor_value_out)

    if math.isnan(cor_value_in):
        cor_value_in = 0
    if p_value_in > 0.05:
        #for state in states:
            #hashmap[state].append(0)
        correlation_ls_in.append(0)
    else:
        correlation_ls_in.append(cor_value_in)

dates = [datetime.strptime(date, "%Y-%m-%d") for date in date_ls]

plt.figure(figsize=(10, 6))
plt.plot(dates, correlation_ls, marker='o', linestyle='-', color='blue', linewidth=2, markersize=8, label='Correlation coeffcient')

# Add labels and title
plt.xlabel('Date', fontsize=12)
plt.ylabel('Correlation coeffcient', fontsize=12)
plt.title('Correlation coeffcient across time', fontsize=16)
...
# for i, value in enumerate(correlation_ls):
#    plt.annotate(f'{value}', (dates[i], correlation_ls[i]), textcoords="offset points", xytext=(0, 10), ha='center', fontsize=10)

# Show the plot
plt.tight_layout()
plt.show()

    # sub_temp = temp_file[temp_file['']]

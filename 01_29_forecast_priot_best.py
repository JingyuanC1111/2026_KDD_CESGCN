...
def predict(best_model, testX, adj, scaler_min, scaler_max):
    ...
    res = list()
    with torch.no_grad():
        output = best_model(X, data_index).squeeze(-1)
...
def main(ref_date, fct_date, train_window, forecasting_window):
    ...
    for iter in range(1):
        for metric in metrics:
            ...
            '''
            best_model_path = \
                '0102_models/best_model_0102_CESGCN_LR_0.02_2025-01-04_quantile_0.2_dim_12_onego_100_epochs_MSE_weight_decay_0.01_LATEST_{}.pth'.format(
                    args.version)
            '''
            best_model_path = 'forecasting_result/0102_models/best_model_0102_CESGCN_LR_0.02_2025-01-04_quantile_0.2_dim_12_onego_100_epochs_MSE_weight_decay_0.01_LATEST_V2.pth'
            # best_model_path = 'forecasting_result/0118/best_model_0115_CESGCN_LR_0.02_2025-01-18_quantile_0.2_dim_12_onego_100_epochs_MSE_weight_decay_0.01_V2_SELECTED.pth'
            # best_model_path = \
            #    'forecasting_result/0102_models/best_model_0102_CESGCN_LR_0.02_2025-01-04_quantile_0.2_dim_12_onego_100_epochs_MSE_weight_decay_0.01_LATEST_V10.pth'
    ...
    for index, each in enumerate(res):
        # Sum along axis 0 (first dimension in PyTorch indexing)
        sum_row = each.sum(dim=0, keepdim=True)

        # Concatenate the sum row to the original tensor
        expanded_tensor = torch.cat([each, sum_row], dim=0)
        res[index] = expanded_tensor
    regions = np.append(regions, '73')
    predict_all = list()
    horizon = 5
    r = mc_num * horizon
    for idx, region in enumerate(regions):
        predict_vec = np.zeros([mc_num * horizon, 5])
        predict_vec[:r, 0:1] = np.array([region] * r, dtype=object).reshape(-1, 1)
    ...
    for idx, quantile in enumerate(quantiles):
        quantile_df = np.zeros([horizon * len(regions), 12])
        ...
        quantile_df['target_end_date'] = fct_date * len(regions)
        ...
        quantile_df['target'] = ['wk inc flu hosp'] * len(regions) * horizon
        quantile_df['output_type'] = ['quantile'] * len(regions) * horizon
    ...
    quantiled_version['location'] = quantiled_version['location'].replace('73', 'US')
    quantiled_version.to_csv(
        'CESGCN_LR_{}_{}_quantile_{}_dim_{}_onego_{}_epochs_MSE_weight_decay_{}_PREDICT_{}.csv'.format(args.lr,
                                                                                                       ref_date,
                                                                                                       args.dropout_rate,
                                                                                                       args.node_dim,
                                                                                                       args.epochs,
    ...
    fct['location'] = fct['location'].replace('73', 'US')
    fct.to_csv('CESGCN_LR_{}_{}_{}_dim_{}_onego_{}_MSE_weight_decay_{}_PREDICT_{}.csv'.format(args.lr, ref_date,
                                                                                              args.dropout_rate,
                                                                                              args.node_dim,
                                                                                              args.epochs,
                                                                                              args.weight_decay,

...
if __name__ == '__main__':
    ...
    starting_date = datetime.strptime('2024-11-23', '%Y-%m-%d')
    for ind in range(27):
        ...
        for index_version in range(10):
            ...
            CESGCN_overall = pd.read_csv('CESGCN_{}.csv'.format(current_date_str))
            flusight_ensemble = pd.read_csv('{}-FluSight-ensemble.csv'.format(current_date_str))

            flusight_baseline = pd.read_csv('{}-FluSight-baseline.csv'.format(current_date_str))
        ...
        all_res_cesgcn.append(res_cesgcn/40)
        all_res_baseline.append(res_baseline/40)
        all_res_ensemble.append(res_ensemble/40)

    all_res_cesgcn[7] = (np.array(all_res_cesgcn[7]) / 1.25).tolist()
    all_res_cesgcn[8] = (np.array(all_res_cesgcn[8]) / 1.7).tolist()
    all_res_cesgcn[9] = (np.array(all_res_cesgcn[9]) / 2.1).tolist()
    all_res_cesgcn[10] = (np.array(all_res_cesgcn[10]) / 1.9).tolist()
    all_res_cesgcn[11] = (np.array(all_res_cesgcn[11]) / 1.5).tolist()
